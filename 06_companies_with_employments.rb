...
crm[:companies].each {|number, comp|
  puts "#{comp}"
  crm[:people].each {|person|
    person[:employments].each {|emp|
      if emp[:company_id] == number
        puts "\t#{person[:first_name]} #{person[:last_name]} - #{emp[:title]}"
      end
    }
  }
}
